...
public class GaussSidelIterative {
	public static void main(String[] args) {
		...
		if (flag0) {
			...
			for (int i = 0; i < niter + 2; i++) {
				...
				// GaussSidel iterative step formula
				iter[0] = (matrix[0][3] - (matrix[0][1] * iter[1]) - (matrix[0][2] * iter[2])) / matrix[0][0];
				round(iter, 0);
				iter[1] = (matrix[1][3] - (matrix[1][0] * iter[0]) - (matrix[1][2] * iter[2])) / matrix[1][1];
				round(iter, 1);
				iter[2] = (matrix[2][3] - (matrix[2][0] * iter[0]) - (matrix[2][1] * iter[1])) / matrix[2][2];
				round(iter, 2);
